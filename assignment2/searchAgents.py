...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        if ((state[1][0]) and (state[1][1]) and 
             (state[1][2]) and (state[1][3])):
             return True
        else:
            return False
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                bottom_left = (nextx, nexty) == self.corners[0]
                top_left = (nextx, nexty) == self.corners[1]
                bottom_right = (nextx, nexty) == self.corners[2]
                top_right = (nextx, nexty) == self.corners[3]
                nextState = ((nextx, nexty), (state[1][0] or bottom_left, state[1][1] or top_left, state[1][2] or bottom_right, state[1][3] or top_right))
...
def cornersHeuristic(state, problem):
    ...
    for i in range(0, 4):
        xy_2 = corners[i]
        state_list = [True, state[1][1], state[1][2], state[1][3]]
        if state[1][i] is False:
            state_list[0], state_list[i] = state_list[i], state_list[0] #swap the position
            next_state = (corners[0], state_list)
            next_state_h = cornersHeuristic(next_state, problem)
            value_list.append(abs(xy_1[0] - xy_2[0]) + abs(xy_1[1] - xy_2[1]) + next_state_h) 
...
def foodHeuristic(state, problem):
    ...
    def distClose(position, foodGrid):
        ...
        for i in range(gridInfo[0]):
            for j in range(gridInfo[1]):
                if foodGrid[i][j]:
                    distance = ((abs(position[0] - i) + abs(position[1] - j)), (i,j))
                    if value is None:
                        value = distance
    ...
    def distCloseFromList(position, foodList):
        ...
        for food in foodList:
            distance = ((abs(position[0] - food[0]) + abs(position[1] - food[1])), food)
            if value is None:
                value = distance
            if distance[0] < value[0]:
                value = distance
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        fringe = util.Queue()
        start_state = (problem.getStartState(), dict())
        ...
        while True: 
            ...
            if not next_node[0] in explored:
                explored.add(next_node[0])
                successors = problem.getSuccessors(next_node[0])
                for i in range(0, len(successors)):
                    if problem.isGoalState(next_node[0]):
                        return next_node[1]
                    path = list(next_node[1])
